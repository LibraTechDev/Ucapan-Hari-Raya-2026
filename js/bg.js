...
// ── Petal particles ──────────────────────────────────────────
const PETAL_COLORS = ['#f9c5d1','#f7b8ca','#fce4ee','#f4a8bf','#fde8f4','#f0b8cc','#fad0dc'];
const pets = [];
...
function frame() {
  ...
  stars.forEach(s => {
    s.ph += s.sp;
    const a = .25 + Math.sin(s.ph) * .25;
    const r = 2.5 + Math.sin(s.ph) * .5;
    ctx.save();
    ctx.globalAlpha = a;
  ...
  pets.forEach((p, i) => {
    ...
    if (p.y > H + 20) { pets[i] = newPetal(); return; }
    ctx.save();
    ctx.globalAlpha = p.a;
...
(function () {
  const positions = [
    [8,12],[22,6],[78,9],[91,15],[4,42],[95,38],
    [12,72],[88,68],[32,88],[68,85],[50,4],[48,95],[20,55],[80,50],
  ];
  positions.forEach(([l, t], i) => {
    const s = document.createElement('div');
    s.className = 'spk';
    s.style.cssText = `left:${l}%;top:${t}%;width:8px;height:8px;animation-delay:${i * .22}s;animation-duration:${2 + Math.random() * 1.5}s`;
